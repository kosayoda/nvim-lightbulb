...
local function update_extmark(opts, position, bufnr)
  ...
  if not (sign_enabled or virt_text_enabled) or position == nil then
    if extmark_id ~= nil then vim.api.nvim_buf_del_extmark(bufnr, LIGHTBULB_NS, extmark_id) end
    return
  end
  ...
  local extmark_opts = {
    ...
    virt_text_win_col = (virt_text_enabled and type(opts.virtual_text.pos) == "number") and opts.virtual_text.pos or nil,
  ...
  vim.b[bufnr].lightbulb_extmark = vim.api.nvim_buf_set_extmark(bufnr, LIGHTBULB_NS, position.line, position.col + 1,
    extmark_opts)
end
...
NvimLightbulb.update_lightbulb = function(config)
  ...
  vim.b[bufnr].lightbulb_lsp_cancel = vim.lsp.buf_request_all(
    bufnr, "textDocument/codeAction", params, handler_factory(opts, position, bufnr)
  )
end
